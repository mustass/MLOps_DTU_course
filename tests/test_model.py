...
def test_model(config_path='config/config.yml'):
    ...
    assert outputs.shape == (config['training']['batch_size'],len(datasets)), 'The output shape is not correct, expected shape {} but got shape {}'.format([config['training']['batch_size'],len(datasets)], [shap for shap in outputs.shape])